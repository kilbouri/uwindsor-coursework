...
int main(void) {
	while (1) {
		...
		puts(OPTION "  1) Addition in SM" RESET);
		puts(OPTION "  2) Subtraction in SM" RESET);
		...
		switch (selected) {
			case 1:
				overflowed = func_signed_mag_addition(first, second, result);
				break;
			case 2:
				overflowed = func_signed_mag_subtraction(first, second, result);
				break;
		}
